...
class VADModel:
    ...
    def detect_speech(
        ...
        try:
            self.logger.logger.info("Starting VAD processing")
            ...
            self.logger.logger.info(
                f"Detected {len(speech_timestamps)} speech segments: {speech_timestamps[:log_speech_max_length]}{'...' if len(speech_timestamps) > log_speech_max_length else ''}"
            )
