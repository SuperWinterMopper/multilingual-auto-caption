...
class VADPipelineTester(PipelineTester):
    ...
    def btest_preprocess_data(self, pipeline) -> None:
    ...
    def btest_split_data(self, pipe) -> None:
    ...
    def btest_train(self, pipe) -> None:
        assert pipe.model is not None, "Model is not initialized."
        assert pipe.trainer is not None, "Trainer is not initialized."

        assert hasattr(pipe, "preprocessed_files"), "Pipeline missing preprocessed_files list."
        assert pipe.preprocessed_files is not None, "Pipeline preprocessed_files is None."
        assert len(pipe.preprocessed_files) > 0, "Pipeline preprocessed_files is empty."

        missing = [str(p) for p in pipe.preprocessed_files if not p.exists()]
    ...
    def atest_preprocess_data(self, pipe) -> None:
        assert pipe.X_train is not None and pipe.y_train is not None, "Training data not set"
        assert pipe.X_valid is not None and pipe.y_valid is not None, "Validation data not set"
        assert pipe.X_test is not None and pipe.y_test is not None, "Test data not set"
    ...
    def atest_split_data(self, pipe) -> None:
