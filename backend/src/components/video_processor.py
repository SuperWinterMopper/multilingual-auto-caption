...
class VideoProcessor:
    ...
    def extract_audio(
        self, video: VideoFileClip, allowed_sample_rates: list[int]
        ...
        try:
            ...
            if audio_tensor.ndim == 2:
                ...
                self.logger.logger.debug(
                    f"Converted stereo audio to mono for {video.filename}"
                )
            else:
                self.logger.logger.debug(
                    f"Video audio is already mono for {video.filename}"
                )

            # Resample if necessary
            original_sample_rate = video.audio.fps
            allowed_sample_rates.sort()
            target_sample_rate_ind = max(
                bisect_right(allowed_sample_rates, original_sample_rate) - 1, 0
            )
            ...
            if original_sample_rate != target_sample_rate:
                ...
                self.logger.logger.info(
                    f"Resampled audio from {original_sample_rate} Hz to {target_sample_rate} Hz for {video.filename}"
                )

            self.logger.logger.info(
                f"Extracted audio: shape={audio_tensor.shape}, sample_rate={target_sample_rate}"
            )
    ...
    def segment_audio(
        ...
        assert len(result) == len(segments), (
            "Number of audio segments does not match number of segments"
        )
        self.logger.logger.info(
            f"Segmented audio into {len(result)} segments for file {orig_file}"
        )
        return result

    def chunk_segments(self, audio_segments: list[AudioSegment]) -> list[AudioSegment]:
        assert all(seg.audio.ndim == 1 for seg in audio_segments), (
            "All audio segments must be mono"
        )
        self.logger.logger.info(
            f"Chunking {len(audio_segments)} audio segments with max duration {self.max_caption_duration} seconds"
        )
        ...
        def chunk_segment(seg: AudioSegment) -> list[AudioSegment]:
            try:
                ...
                for chunk in range(num_chunks):
                    start_idx = int(
                        (chunk * self.max_caption_duration) * seg.sample_rate
                    )
                    end_idx = int(
                        ((chunk + 1) * self.max_caption_duration) * seg.sample_rate
                    )

                    start_time = seg.start_time + chunk * self.max_caption_duration
                    end_time = min(
                        seg.start_time + (chunk + 1) * self.max_caption_duration,
                        seg.end_time,
                    )
        ...
        for seg in audio_segments:
            ...
            assert chunked[0].start_time == seg.start_time, (
                "Chunking altered start time"
            )
            assert chunked[-1].end_time == seg.end_time, "Chunking altered end time"

            # extra checking only in dev mode
            if not self.prod:
                assert sum(c.audio.numel() for c in chunked) == seg.audio.numel(), (
                    "Chunking altered total audio samples"
                )
                assert np.isclose(
                    sum(c.end_time - c.start_time for c in chunked),
                    (seg.end_time - seg.start_time),
                ), "Chunking altered total duration"
            new_segments.extend(chunked)

        self.logger.logger.info(
            f"Finished chunking into {len(new_segments)} segments from {len(audio_segments)} original segments"
        )
        return new_segments
    ...
    def pick_font_for_text(self, text: str) -> str:
        def font_supports_text(font_path: str, text: str) -> bool:
            if not text:  # if text is empty any font will do
                self.logger.logger.warning(
                    f"Text is empty so deeming font is valid for text:<{text}> at {font_path}"
                )
                return True
            try:
                tt = TTFont(font_path)
                ...
                self.logger.logger.error(
                    f"Error occured in font_supports_text: {e}. Deeming that "
                )
                return True
        ...
        self.logger.logger.warning(
            f"No font found that supports all characters in text: {text[:50]}. Defaulting to {default_choice}."
        )
    ...
    def embed_captions(
        ...
        if audio_segments is None:
            raise ValueError(
                "audio_segments is None - audio processing may have failed"
            )

        # Debug logging for None checks
        self.logger.logger.info(
            f"embed_captions called with video={video}, audio_segments type={type(audio_segments)}, len={len(audio_segments)}"
        )

        self.logger.logger.info(
            f"Embedding {len(audio_segments)} captions ({audio_segments[0].text}...) into video {video.filename}"
        )

        assert all(seg.end_time <= video.duration for seg in audio_segments), (
            "All audio segments must have end time within video duration"
        )
        assert all(seg.lang != unknown_language for seg in audio_segments), (
            "All segments must have known language before embedding captions"
        )
        assert all(seg.text != unknown_text for seg in audio_segments), (
            "All segments must have known text before embedding captions"
        )
        ...
        try:
            for seg in audio_segments:
                ...
                try:
                    ...
                    txt_clip = txt_clip.with_start(seg.start_time).with_duration(
                        duration
                    )
                except Exception as e:
                    self.logger.logger.error(
                        f"Error occured in indiviaul TextClip creation: {e}"
                    )
                    raise
            ...
            self.logger.logger.error(
                f"Error occured during creation of TextClips for embedding into video: {e}"
            )
            raise

        try:
            final_video = CompositeVideoClip(
                [video] + text_clips, size=(video.w, video.h)
            )
        except Exception as e:
            self.logger.logger.error(
                f"Error occured during creation of CompositeVideoClip: {e}"
            )
            raise

        self.logger.logger.info(
            f"Successfully embedded {len(text_clips)} captions into video"
        )
        return final_video
