...
class AppLogger:
    def __init__(self, log_suffix, level: int = logging.INFO, prod=False):
        ...
        self.prod = prod
        self.log_root, self.log_file = self.create_log_directory(
            logs_root=self.logs_dir, log_suffix=log_suffix
        )
        ...
        self._heartbeat_thread = threading.Thread(
            target=self.heartbeat_metrics, args=(60,), daemon=True
        )
    ...
    def log_machine_info(self):
        ...
        machine_info = (
            f"CPU: {platform.processor() or platform.uname().processor} | "
    ...
    def create_log_directory(
        self, logs_root: Path, log_suffix: str
    ) -> tuple[Path, Path]:
        logs_root.mkdir(parents=True, exist_ok=True)

        root = (
            logs_root
            / f"{datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}_{log_suffix}"
        )
    ...
    def stop(self):
        ...
        if self._heartbeat_thread.is_alive():
            self.logger.info(
                "Stopping heartbeat thread (should have no more heartbeats)"
            )
    ...
    def log_video_metrics(self, video: VideoFileClip):
        self.logger.info(
            f"Video Metrics: duration={video.duration}s, fps={video.fps}, size={video.size}"
        )

    def log_segments_visualization(
        self, log_prefix: str, video: VideoFileClip, audio_segments: list[AudioSegment]
    ):
        if self.prod:
        ...
        self.logger.info(
            f"Creating segments visualization for {len(audio_segments)} segments..."
        )
        ...
        if not all(seg.orig_file == orig_file for seg in audio_segments):
            self.logger.warning(
                "Not all audio segments have the same orig_file. Skipping visualization."
            )
            return
        ...
        lang_segments: dict[str, list[tuple[float, float, int]]] = {
            lang: [] for lang in langs
        }
        for i, seg in enumerate(audio_segments, start=1):
            lang_segments[seg.lang].append(
                (seg.start_time, seg.end_time - seg.start_time, i)
            )
        ...
        for lang, segments in lang_segments.items():
            ...
            for xr, yr in zip(xranges, yranges):
                ax.broken_barh(
                    [xr], yr, facecolors=lang_to_color[lang], edgecolors="black"
                )

        # Create legend
        handles = [mpatches.Patch(color="lightgray", label="Video Duration")]
        handles.extend(
            mpatches.Patch(color=lang_to_color[lang], label=lang) for lang in langs
        )
        ...
        ax.set_title(
            f"Audio Segments: {orig_file[:max_title_len]}{'...' if len(orig_file) > max_title_len else ''}"
        )
        ax.set_ylim(-0.5, len(audio_segments) + 0.5)
        ax.set_xlim(0, video.duration)
        ax.set_yticks(range(len(audio_segments) + 1))
        ax.set_yticklabels(
            ["Video"] + [f"Segment {i}" for i in range(1, len(audio_segments) + 1)]
        )

        # Save figure with optimized settings
        viz_path = (
            self.log_root
            / f"{log_prefix}_vis_{datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.png"
        )
    ...
    def log_audio_segments_list(self, audio_segments: list[AudioSegment]):
        ...
        def format_segment(seg: AudioSegment, idx: int) -> str:
            ...
            return (
                f"[{idx}] id={seg.id} | {seg.start_time:.2f}s-{seg.end_time:.2f}s | "
    ...
    def log_transcription_results(
        self, audio_segments: list[AudioSegment], log_prefix: str
    ):
        if self.prod:
            return  # skip in prod mode
        ...
        results_path = (
            self.log_root
            / f"{log_prefix}_{audio_segments[0].lang}_{datetime.datetime.now().strftime('%Y_%m_%d_%H_%M_%S')}.txt"
        )
        ...
        self.logger.info(
            f"Transcription results saved to {results_path}. The language of the first segment is part of the filename, not necessarily all languages"
        )
